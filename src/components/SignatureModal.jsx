...
// In-app signing, for when the técnico hands their own device to the client
// right there on site. Same capture/validation/save as the public /firma/:id
// link (shared via useSignatureFlow) — this is just that flow as a modal
// over ReportDetail instead of a full page, for when there's no need to
// email a link and wait for the client to open it separately.
export default function SignatureModal({ reportId, onClose, onSigned }) {
  ...
  // Once saved, let the técnico see the confirmation for a moment, then
  // hand control back to ReportDetail so it can refresh the report status.
  useEffect(() => {
    if (!saved) return
    ...
    >
      <div className="card fade-in" style={{ maxWidth: 560, width: '100%', maxHeight: '90vh', overflowY: 'auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
          <p className="section-tag" style={{ marginBottom: 0 }}>Firmar Reporte</p>
